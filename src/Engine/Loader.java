...
public class Loader {
	...
	public RawModel loadToVAOSquare(float[] positions,  int[] indices) {
		int vaoID = createVAO();
		bindIndicesBuffer(indices);
		storeDataInAttributeList2(0, positions);
		unbindVAO();
		return new RawModel(vaoID, indices.length);
	}
	...
	public int loadTexture(String fileName){
		...
		try {
			texture = TextureLoader.getTexture("PNG", new FileInputStream("src/assets/" + fileName + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void storeDataInAttributeList2(int attributeNum, float[] data) {
		int vboID = GL15.glGenBuffers();
